...
export async function authorForgotPassword() {
  ...
  container.innerHTML = `
    <div class="max-w-md w-full">
      <div class="text-center mb-8">
        <i class="fa-solid fa-key text-5xl text-purple-600 mb-4"></i>
  ...
  form.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      const result = await store.requestAuthorPasswordReset(email);
      ...
      if (result.token) {
        ...
        console.log('Reset URL:', `${window.location.origin}/#author-reset-password?token=${result.token}`);
